...
function editor_style_callback(a, b) {
  switch (a) {
    case "crop":
      ...
      break;
    case "undo":
      sendMessage({ type: "canUndo", canUndo: b });
      break;
    case "redo":
...
window.addEventListener("message", function (a) {
  if (a.data.type == "init") initEditor(a.data.dataUrl, a.data.initTool);
    ...
    sendMessage({ type: "saveWithTag", dataURL: a });
  } else if (a.data.type == "saveAttach") {
    a = editor.getImageDataURL();
    sendMessage({ type: "saveAttach", dataURL: a });
  }
  sendMessage({ type: "canUndo", canUndo: editor.history.canUndo() });
});
